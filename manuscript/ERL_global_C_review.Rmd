...
Remote sensing technology has become increasingly useful for global- or regional-scale estimates of a few critical variables [e.g., aboveground biomass: @hu_mapping_2016; @spawn_harmonized_2020; gross primary productivity, $GPP$: @li_mapping_2019; @saatchi_benchmark_2011], yet measurement and validation of most forest C stocks and fluxes necessarily requires intensive on-the-ground data collection.
...
Since *(Humboldt?)*, it has been recognized that climate plays a dominant role in shaping differences among forests.
Early quantitative syntheses revealed that C fluxes such as net primary productivity ($NPP$) decrease dramatically with latitude, or increase across gradients of mean annual temperature and, to a lesser extent, precipitation [@lieth_primary_1973].

@banbury_morgan_global_nodate

Recently, machine learning draws on global maps in combination with ground-based data to create global maps of C cycling [e.g., REFS; @cook-patton_mapping_2020].

While current data and technology allow increasingly fine-scale resolution for understanding differences across forests, cateogorization into biomes remains useful for applications where data do not yet support robust analysis across finer-scale axes of variation.

### Age trends and their variation across biomes
In 1969, Eugene Odum published ... [@odum_strategy_1969].
Although the conceptualization of the C cycle in this paper is simplistic by current standards, the paper was foundational... [@corman_foundations_2019].
...
*(Need for a synthetic update.)*
...
Given the important role of secondary forests in the current and future global C cycle, concrete understanding of age trends in C fluxes and stocks and how these vary across biomes is critical to better understandign of the global C cycle.

```{r schematic, eval = generate_figures, echo=FALSE, fig.align = "center", fig.cap = "Figure 1 | DRAFT Schematic diagram summarizing current understanding of biome differences and age trends in forest C cycling. Plotted values are realistic with respect to the results of this synthesis, but schematic is not intended to be quantitatively precise. ", out.width='100%', fig.pos='H'}
...
Both C stocks and fluxes commonly increased significantly with stand age (Tables 1, S2, Figs. `r flux_age_trends`- `r stock_age_trends`, `r TrB_age_trends`-`r BoN_age_trends`, `r all_age_trends`; detailed below).

*ForC* contained 16 C flux variables with sufficient data for analyses of age trends in young forests (see Methods) (Figs. `r flux_age_trends`, `r all_flux_age_trends`).
...
In contrast, the  CO~2~ influx portion of the budget generally did not "close": the sum of $R_{auto}$ ($R_{root}+R_{auto-ag}$, as described above) and components of $NPP$ consistently fell short of $GPP$ by **##-##** Mg C ha^-1^ yr^-1^ , particularly in in young stands.
