...
On average, aboveground biomass ($B_{ag}$) tends to decrease with latitude, but not as dramatically as fluxes, and with the highest $B_{ag}$ forests in relatively cool, moist temperate regions [@keith_reevaluation_2009; @smithwick_potential_2002; @hu_mapping_2016]. 
...
This is a critical limitation not only for understanding forest C cycling, but also for quantifying forest-based climate change mitigation potential across forest biomes or ecozones [e.g., @ipcc_2019_2019].
...
While these trends have been subject of fairly recent qualitative review [@anderson-teixeira_altered_2013], there is need for a synthetic, quantitative review taking advantage of the greatly expanded data now available.
...
Most recently, @cook-patton_mapping_2020 reinforced these earlier findings with a much larger dataset and created a high-resolution global map of estimated potential C accumulation rates.
...
As such, there has been a focus on incorporating data from regrowth forests [*e.g.*, @anderson_temperaturedependence_2006; @martin_carbon_2013; @bonner_metaanalytical_2013] and obtaining stand age data when possible [83% of records in v.2.0, @anderson-teixeira_forc_2018].
...
Note that two flux variables, aboveground heterotropic respiration ($R_{het-ag}$) and total heterotropic respiration ($R_{het}$), were included for conceptual completeness but had no records in *ForC* (Table 1).
...
Assigning stands to these groupings required the exclusion of records for which *ForC* lacked geographic coordinates (`r round(n_sites_coord_unk/nrow(ForC_sites)*100, digits=1)`% of sites in full database) or records of stand age or forest maturity (`r round(n_records_age_unk/nrow(ForC_measurements)*100, digits=1)`% of records in full database). 
...
We calculated means and standard deviations of each mature forest C cycle variable by biome over geographically distinct areas to produce biome-specific schematics.
...
When biome had a significant effect, we used a Tukey's pairwise comparison to see which biomes were significantly different from one another. 
...
This analysis was run for variables and biomes with records for at least three distinct geographic areas per biome, excluding any biomes that failed this criteria (Table 1).
