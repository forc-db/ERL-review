...
Despite the centrality of forest C cycling in regulating atmospheric CO~2~, important uncertainties in climate models [@friedlingstein_climate_2006; @krause_large_2018; @bonan_model_2019; @divittorio_initial_2020] and CO~2~ accounting frameworks [@pan_large_2011; @ipcc_2019_2019] can be traced to lack of understanding on how C cycling varies across forest types and in relation to stand history.
...
Yet, measurement and validation of most forest C stocks and fluxes necessarily requires intensive on-the-ground data collection.
...
On average, aboveground biomass ($B_{ag}$) tends to decrease with latitude, but not as dramatically as fluxes, and with the highest $B_{ag}$ forests in relatively cool, moist temperate regions [@smithwick_potential_2002; @keith_reevaluation_2009; @hu_mapping_2016]. 
...
Here, we conduct a data-based review of carbon cycling from a stand to global level, and by biome and stand age, using our open-source Global Carbon Forest database (*ForC*; Fig. `r figure_map`), which is the largest global compilation of forest carbon data.
...
*ForC* contained 16 C flux variables with sufficient data to analyize age trends in young forests (see Methods).
...
All of these displayed a significant overall increase with $log10[stand.age]$.
...
Specifically, most C fluxes were highest in tropical forests, intermediate in temperate (broadleaf or conifer) forests, and lowest in boreal forests -- a pattern that generally held for both regrowth and mature forests (Figs. `r schematic`a, `r flux_age_trends`- `r stock_age_trends`, `r age_trends_all`).
The notable exception was mature forest $NEP$ (the difference between $GPP$ and $R_{eco}$) which was statistically indistinguishable across biomes (Fig. `r flux_age_trends`f).
There was also little directional variation in mean mature forest C stocks (biomass, dead wood, and organic layer) across biomes. However, maximum values for most stocks (all including live or standing woody biomass) occurred in temperate biomes (Figs. `r schematic`c, `r all_diagrams_mature`, `r stock_age_trends`).
...
Thus, moving from cold to tropical climates and from young to old stands, there is a general acceleration of C cycling. In contrast, C stocks and $NEP$ of mature forests, which are defined by the differences between in- and out- fluxes, do not vary systematically across biomes.
Together, these results refine and expand our understanding of C cycling in mature forests, while providing the first global-scale analysis of age trends in multiple forest C stocks and fluxes (Figs. `r age_trends_all`). 

### C cycling across biomes

Our analysis reveals that carbon cycling is most rapid in the tropics and slowest in boreal regions.
...
The finding that these patterns hold consistently across numerous fluxes, while consistent with theoretical expectations (Fig. `r schematic`a), is novel to this analysis [but see @banburymorgan_global_inpress for nine autotrophic fluxes].
...
Unlike the other C flux variables, $NEP$ does not characterize the rate at which C cycles through the ecosystem, but, as the balance between $GPP$ and $R_{eco}$, represents net CO~2~ sequestration (or release) by the ecosystem (Fig. `r schematic`a-b).
...
However, $NEP$ is theoretically expected to peak in intermediate-aged stands and thereafter decline, consistent with decelerating C accumulation as stands age [Fig. `r age_trends_all`, @odum_strategy_1969], and such declines have been observed [@law_changes_2003; @luyssaert_oldgrowth_2008].
...
In terms of stocks, our study reveals consistent increases in live biomass stocks with stand age, a pattern that is well-known and expected [e.g., @lichstein_biomass_2009; @yang_carbon_2011], contrasting with more variable age trends in deadwood and the organic layer (Fig. `r age_trends_all`).
The latter depend strongly on the type of disturbance. 
Disturbances that remove most woody material (e.g., clearcut logging, agriculture) result in negligible deadwood in young stands, followed by a buildup over time [e.g., tropical stands in Figs. `r stock_age_trends`, `r age_trends_all`; e.g., @vargas_biomass_2008].
In contrast, natural disturbances (e.g., fire, drought, typhoons/hurricanes) can produce large amounts of deadwood (mostly $DW_{standing}$) that slowly decomposes as the stand recovers, resulting in declines across young stand ages [e.g., temperate and boreal stands in Figs. `r stock_age_trends`, `r age_trends_all`; e.g., @carmona_coarse_2002].
...
However, there is wide variation around these means, implying that any given stand could deviate substantially from them, and the sample means presented here probably do not represent true biome means (particularly for temperate conifer forests where high-biomass stands are over-represented in *ForC*).
...
Notably, *ForC* currently has sparse--if any--coverage of fluxes to herbivores and higher consumers, along with woody mortality ($M_{woody}$) and $DW$ (Table 1, Figs. `r DW_tot_fig`-`r DW_down_fig`). 
...
Ground-based data such as *ForC* are therefore valuable in defining age-based trajectories in biomass, as in @cook-patton_mapping_2020, and thus constraining variables such as carbon sink potential [@luyssaert_oldgrowth_2008].
