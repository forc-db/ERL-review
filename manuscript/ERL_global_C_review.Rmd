...
#Table 1 for this manuscript
T1 <- read.csv("tables_figures/C_variables.csv", stringsAsFactors = FALSE, check.names = FALSE)
n_records <- sum(T1[,"records"])

#ForC raw
ForC_measurements <- read.csv("/Users/kteixeira/Dropbox (Smithsonian)/GitHub/ForC-db/ForC/data/ForC_measurements.csv", stringsAsFactors = FALSE, check.names = FALSE)
ForC_plots <- read.csv("/Users/kteixeira/Dropbox (Smithsonian)/GitHub/ForC-db/ForC/data/ForC_plots.csv", stringsAsFactors = FALSE, check.names = FALSE)
ForC_sites <- read.csv("/Users/kteixeira/Dropbox (Smithsonian)/GitHub/ForC-db/ForC/data/ForC_sites.csv", stringsAsFactors = FALSE, check.names = FALSE)
n_areas <- max(ForC_sites[,"geographic.area"], na.rm = TRUE )

#ForC simplified
ForC_simplified <- read.csv("/Users/kteixeira/Dropbox (Smithsonian)/GitHub/ForC-db/ForC/ForC_simplified/ForC_simplified.csv", stringsAsFactors = FALSE, check.names = FALSE)
...
records_eligible_for_inclusion <- NA  #number in ForC_simplified that are not disturbed, managed, or no.history

#stats I haven't yet acquired:
n_records_age_unk <- NA  #total n records no age info
n_records_coord_unk <- NA  #total n records no geographic coordinates

#total numbers of plots and areas (all variables combined)
n_plots_tot <- NA
n_areas_tot <- NA

# percent eligible for inclusion in analysis

n_focal_variable_records_eligible_for_inclusion <- NA #This is prior to assigning biome groupings
 
# records by biome grouping
n_records_BoB <- NA
n_records_Bomixed <- NA
n_records_Te_mixed <- NA

#numbers of variables with records from ≥7 geographic areas, by biomes/ age class:
n_fluxes_matureTrB_with_ge7_areas <- NA
n_stocks_matureTrB_with_ge7_areas <- NA
n_fluxes_matureTeB_with_ge7_areas <- NA
n_stocks_matureTeB_with_ge7_areas <- NA
n_fluxes_matureTeN_with_ge7_areas <- NA
n_stocks_matureTeN_with_ge7_areas <- NA
n_fluxes_matureBoN_with_ge7_areas <- NA
n_stocks_matureBoN_with_ge7_areas <- NA

n_fluxes_youngTrB_with_ge7_areas <- NA
n_stocks_youngTrB_with_ge7_areas <- NA
n_fluxes_youngTeB_with_ge7_areas <- NA
n_stocks_youngTeB_with_ge7_areas <- NA
n_fluxes_youngTeN_with_ge7_areas <- NA
n_stocks_youngTeN_with_ge7_areas <- NA
n_fluxes_youngBoN_with_ge7_areas <- NA
n_stocks_youngBoN_with_ge7_areas <- NA
...
Assigning stands to these groupings required the exclusion of records for which ForC lacked records of stand age (n=`r round(n_records_age_unk/nrow(ForC_measurements)*100, digits=1)`% of total database), or geographic coordinates (n=`r round(n_records_coord_unk/nrow(ForC_measurements)*100, digits=1)`% of total database). 
...
The most well-represented forest types excluded were boreal broadleaf (`r n_records_BoB` records) and boreal and temperate mixed broadleaf-needleleaf (`r n_records_Bomixed` and `r n_records_Te_mixed` records, respectively).
...
Of the `r nrow(ForC_measurements)` in ForC v.#, `r n_records` met the criteria for inclusion in this study (Fig. 1). 
These records were distributed across `r n_plots_tot` plots in `r n_areas_tot` distinct geographic areas. 
Of the 23 flux and 11 stock variables mapped in these diagrams, ForC contained mature forest estimates from $\ge$ 7 distinct geographic areas for `r n_fluxes_matureTrB_with_ge7_areas` fluxes and `r n_stocks_matureTrB_with_ge7_areas` stocks in tropical broadleaf forests, `r n_fluxes_matureTeB_with_ge7_areas` fluxes and `r n_stocks_matureTeB_with_ge7_areas` stocks in temperate broadleaf forests, `r n_fluxes_matureTeN_with_ge7_areas` fluxes and `r n_stocks_matureTeN_with_ge7_areas` stocks in temperate conifer forests, and `r n_fluxes_matureBoN_with_ge7_areas` fluxes and `r n_stocks_matureBoN_with_ge7_areas` stocks in boreal forests. 
For regrowth forests (<100 yrs), ForC contained estimates from $\ge$ 7 distinct geographic areas for `r n_fluxes_youngTrB_with_ge7_areas` fluxes and `r n_stocks_youngTrB_with_ge7_areas` stocks in tropical broadleaf forests, `r n_fluxes_youngTeB_with_ge7_areas` fluxes and `r n_stocks_youngTeB_with_ge7_areas` stocks in temperate broadleaf forests, `r n_fluxes_youngTeN_with_ge7_areas` fluxes and `r n_stocks_youngTeN_with_ge7_areas` stocks in temperate conifer forests, and `r n_fluxes_youngBoN_with_ge7_areas` fluxes and `r n_stocks_youngBoN_with_ge7_areas` stocks in boreal forests. 
...
```{r echo=FALSE, fig.align = "center", fig.cap = "Figure 2 | C cycle diagram for mature tropical broadleaf forests. All units are Mg C ha$^{-1}$ yr$^{-1}$ (fluxes) or Mg C ha$^{-1}$. Presented are mean ± std, where geographically distinct areas are treated as the unit of replication.  Arrows indicate fluxes, boxes indicate stocks. Dashed shape outlines indicate variables with records from <7 distinct geographic areas, and dashed arrows indicate fluxes with no data. Arrows are scaled as the **square root of flux divided by 5**.", out.width='100%', fig.pos='H'}
  ...
  knitr::include_graphics("/Users/kteixeira/Dropbox (Smithsonian)/GitHub/ForC-db/ForC/figures/C_cycle_diagrams/Diagrams/Tropical broadleaf MATURE.png")
```
...
```{r echo=FALSE, fig.align = "center", fig.cap = "Figure 3 | C cycle diagram for mature temperate broadleaf forests. All units are Mg C ha$^{-1}$ yr$^{-1}$ (fluxes) or Mg C ha$^{-1}$. Presented are mean ± std, where geographically distinct areas are treated as the unit of replication.  Arrows indicate fluxes, boxes indicate stocks. Dashed shape outlines indicate variables with records from <7 distinct geographic areas, and dashed arrows indicate fluxes with no data.", out.width='100%', fig.pos='H'}
  ...
  knitr::include_graphics("/Users/kteixeira/Dropbox (Smithsonian)/GitHub/ForC-db/ForC/figures/C_cycle_diagrams/Diagrams/Temperate broadleaf MATURE.png")
```
...
```{r echo=FALSE, fig.align = "center", fig.cap = "Figure 4 | C cycle diagram for mature temperate conifer forests. All units are Mg C ha$^{-1}$ yr$^{-1}$ (fluxes) or Mg C ha$^{-1}$. Presented are mean ± std, where geographically distinct areas are treated as the unit of replication.  Arrows indicate fluxes, boxes indicate stocks. Dashed shape outlines indicate variables with records from <7 distinct geographic areas, and dashed arrows indicate fluxes with no data.", out.width='100%', fig.pos='H'}
  ...
  knitr::include_graphics("/Users/kteixeira/Dropbox (Smithsonian)/GitHub/ForC-db/ForC/figures/C_cycle_diagrams/Diagrams/Temperate conifer MATURE.png")
```
...
```{r echo=FALSE, fig.align = "center", fig.cap = "Figure 5 | C cycle diagram for mature boreal conifer forests. All units are Mg C ha$^{-1}$ yr$^{-1}$ (fluxes) or Mg C ha$^{-1}$. Presented are mean ± std, where geographically distinct areas are treated as the unit of replication.  Arrows indicate fluxes, boxes indicate stocks. Dashed shape outlines indicate variables with records from <7 distinct geographic areas, and dashed arrows indicate fluxes with no data.", out.width='100%', fig.pos='H'}
  ...
  knitr::include_graphics("/Users/kteixeira/Dropbox (Smithsonian)/GitHub/ForC-db/ForC/figures/C_cycle_diagrams/Diagrams/Boreal conifer MATURE.png")
```
\elandscape
...
```{r echo=FALSE, fig.align = "center", fig.cap = "Figure 8 | C cycle diagram for young tropical broadleaf forests. All units are Mg C ha$^{-1}$ yr$^{-1}$ (fluxes) or Mg C ha$^{-1}$. Presented are observed ranges, where geographically distinct areas are treated as the unit of replication.  Arrows indicate fluxes, boxes indicate stocks. Dashed shape outlines indicate variables with records from <7 distinct geographic areas, and dashed arrows indicate fluxes with no data.", out.width='100%', fig.pos='H'}
  ...
  knitr::include_graphics("/Users/kteixeira/Dropbox (Smithsonian)/GitHub/ForC-db/ForC/figures/C_cycle_diagrams/Diagrams/Tropical broadleaf YOUNG.png")
```
...
```{r echo=FALSE, fig.align = "center", fig.cap = "Figure 9 | C cycle diagram for young temperate broadleaf forests. All units are Mg C ha$^{-1}$ yr$^{-1}$ (fluxes) or Mg C ha$^{-1}$. Presented are observed ranges, where geographically distinct areas are treated as the unit of replication.  Arrows indicate fluxes, boxes indicate stocks. Dashed shape outlines indicate variables with records from <7 distinct geographic areas, and dashed arrows indicate fluxes with no data.", out.width='100%', fig.pos='H'}
  ...
  knitr::include_graphics("/Users/kteixeira/Dropbox (Smithsonian)/GitHub/ForC-db/ForC/figures/C_cycle_diagrams/Diagrams/Temperate broadleaf YOUNG.png")
```
...
```{r echo=FALSE, fig.align = "center", fig.cap = "Figure 10 | C cycle diagram for young temperate conifer forests. All units are Mg C ha$^{-1}$ yr$^{-1}$ (fluxes) or Mg C ha$^{-1}$. Presented are observed ranges, where geographically distinct areas are treated as the unit of replication.  Arrows indicate fluxes, boxes indicate stocks. Dashed shape outlines indicate variables with records from <7 distinct geographic areas, and dashed arrows indicate fluxes with no data.", out.width='100%', fig.pos='H'}
  ...
  knitr::include_graphics("/Users/kteixeira/Dropbox (Smithsonian)/GitHub/ForC-db/ForC/figures/C_cycle_diagrams/Diagrams/Temperate conifer YOUNG.png")
```
...
```{r echo=FALSE, fig.align = "center", fig.cap = "Figure 11 | C cycle diagram for young boreal conifer forests. All units are Mg C ha$^{-1}$ yr$^{-1}$ (fluxes) or Mg C ha$^{-1}$. Presented are observed ranges, where geographically distinct areas are treated as the unit of replication.  Arrows indicate fluxes, boxes indicate stocks. Dashed shape outlines indicate variables with records from <7 distinct geographic areas, and dashed arrows indicate fluxes with no data.", out.width='100%', fig.pos='H'}
  ...
  knitr::include_graphics("/Users/kteixeira/Dropbox (Smithsonian)/GitHub/ForC-db/ForC/figures/C_cycle_diagrams/Diagrams/Boreal conifer YOUNG.png")
```
\elandscape
